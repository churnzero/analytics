...
__export(__webpack_require__(/*! ./models/announcement/announcement-style-config */ "./node_modules/@churnzero/clientlibs/dist/main/models/announcement/announcement-style-config.js"));
__export(__webpack_require__(/*! ./models/common/style-config */ "./node_modules/@churnzero/clientlibs/dist/main/models/common/style-config.js"));
...
__export(__webpack_require__(/*! ./constants/style-config/global-style-config */ "./node_modules/@churnzero/clientlibs/dist/main/constants/style-config/global-style-config.js"));
...
var ModelUtils = (function () {
    ...
    ModelUtils.getNonMutatedValue = function (val) {
    ...
    ModelUtils.assignDefined = function (target) {
        ...
        for (var i = 0; i < sl; i++) {
            ...
            for (var prop in source) {
                ...
                target[prop] = ModelUtils.getNonMutatedValue(val);
            }
        }
        return target;
    };
    ModelUtils.create = function (dest, input, strict) {
        ...
        if (input) {
            Object.keys(input).forEach(function (k) {
                ...
                var val = input[k];
                dest[k] = ModelUtils.getNonMutatedValue(val);
            });
        }
        return dest;
    };
...
if (!clientlibs_1.CoreUtils.FeatureDetection.isLegacyIE()) {
    ...
    window["ChurnZero"] = ChurnZeroExt_1.ChurnZeroExt() || {};
...
/***/ "./src/core/DevlessPanelLauncher.ts":
/*!******************************************!*\
  !*** ./src/core/DevlessPanelLauncher.ts ***!
  \******************************************/
/*! no static exports found */
...
var PubSuber_1 = __importDefault(__webpack_require__(/*! @src/core/PubSuber */ "./src/core/PubSuber.ts"));
var DevlessPanelLauncher = /** @class */ (function (_super) {
    __extends(DevlessPanelLauncher, _super);
    function DevlessPanelLauncher(appSvc, tenantSvc, windowSvc, storageSvc, sessionSvc, messageSvc, commSvc, openOnStart) {
    ...
    DevlessPanelLauncher.prototype.compileLauncher = function () {
        var _this = this;
        ...
        this.appSvc.templater.compile_template(this.appSvc.addCdn(this.fileNames.CSMPanelTemplate + '?ak=' + encodeURIComponent(this.commSvc.appKey)), data, function (result) { return _this.buildLauncher(result); });
    };
    DevlessPanelLauncher.prototype.buildLauncher = function (result) {
        var _this = this;
        ...
        this.windowSvc.window.setTimeout(function () {
            try {
                ...
                //this.windowSvc.czq('#cz-panel-container').removeClass('hidden').attr('style', '');
                var basePanel = _this.windowSvc.document.querySelector('#cz-panel-container');
                if (basePanel) {
                    basePanel.classList.remove('hidden');
                    basePanel.attributes.removeNamedItem('style');
                }
                ...
                //report?
                errorLogger_1.default.report('DevlessPanelLauncher.buildCsmPanelTimeout', ex);
            }
        }, 500);
    };
...
(function (Enums) {
    ...
    var WalkThroughElementType;
    (function (WalkThroughElementType) {
        WalkThroughElementType[WalkThroughElementType["PopUp"] = 0] = "PopUp";
        WalkThroughElementType[WalkThroughElementType["Slider"] = 1] = "Slider";
        WalkThroughElementType[WalkThroughElementType["Tooltip"] = 2] = "Tooltip";
    })(WalkThroughElementType = Enums.WalkThroughElementType || (Enums.WalkThroughElementType = {}));
    ...
    })(AnnouncementType = Enums.AnnouncementType || (Enums.AnnouncementType = {}));
    /**
...
var ClickAudit_util_1 = __webpack_require__(/*! ../utils/ClickAudit.util */ "./src/utils/ClickAudit.util.ts");
var Enums_1 = __webpack_require__(/*! ./Enums */ "./src/core/Enums.ts");
...
var FrameBuilder = /** @class */ (function () {
    ...
    FrameBuilder.prototype.buildInAppContentFrame = function (configuration) {
        ...
        var cssText = '', secondaryClassName = 'popup';
        if (fontFamily) {
    ...
    FrameBuilder.prototype.getInAppContentIFrameLoadFunc = function (config, transportPod, customCss) {
        var _this = this;
        var styleConfig = config.styleConfig, content = config.content, width = config.width, type = config.type, position = config.position, onResizeCallback = config.onResizeCallback, onCloseCallback = config.onCloseCallback, onClickCallback = config.onClickCallback;
        // Page wrap click handler
        var clickHandler = function (e) {
            var className = e.target.className, wasClose = className === 'cz-close-in-app';
            if (wasClose && onCloseCallback && typeof onCloseCallback === 'function') {
                onCloseCallback();
                return;
            }
        ...
        return function (e) {
            ...
            */
            ...
            ClickAudit_util_1.ClickAuditUtil.setUpLinkAuditEvents(win, transportPod);
            ClickAudit_util_1.ClickAuditUtil.setUpVideoAuditEvents(win, transportPod);
            //ensure the iframe element on the parent page has the same height as its content inside
            var checkHeight = function () {
                try {
                    var totalHeight = 0;
                    var br = pageWrap.getBoundingClientRect(), bTop = parseInt(contentiFrame.style.borderTopWidth, 10), bBottom = parseInt(contentiFrame.style.borderBottomWidth, 10);
                    totalHeight += br.height;
                    // Take the frame border into account when calculating the total height for the frame
                    if (bTop || 0 > 0) {
                        totalHeight += bTop;
                    }
                    if (bBottom || 0 > 0) {
                    ...
                    contentiFrame.style.height = totalHeight + 'px';
                    typeof onResizeCallback === 'function' && onResizeCallback();
                }
                catch (ex) {
                    console.error(ex);
    ...
    FrameBuilder.prototype.buildWalkthroughContentFrame = function (configuration) {
        var stepType = configuration.stepType, content = configuration.content, config = configuration.config, clickAuditData = configuration.clickAuditData, onResizeCallback = configuration.onResizeCallback, styleConfig = configuration.styleConfig, sequence = configuration.sequence, metaData = configuration.metaData;
        var contentiFrame = this.windowSvc.document.createElement("iframe");
        contentiFrame.style.margin = "0";
        contentiFrame.style.padding = "0";
        contentiFrame.style.border = "none";
        ...
        if (stepType === clientlibs_1.WalkthroughStepType.Slider) {
            contentiFrame.classList.add('slider');
        }
        contentiFrame.classList.add('cz-walk-content-frame');
        contentiFrame.addEventListener('load', this.getWalkthroughContentIFrameLoadFunc(meta, onResizeCallback));
        return contentiFrame;
    };
    FrameBuilder.prototype.getWalkthroughContentIFrameLoadFunc = function (metaData, onResizeCallback) {
        var _this = this;
        ...
        return function (e) {
            ...
            b.appendChild(pageWrap);
            if (contentTypeClass) {
                b.classList.add(contentTypeClass);
            }
            if (!Element.prototype.matches)
                Element.prototype.matches = Element.prototype["msMatchesSelector"];
            if (!Element.prototype.closest)
            ...
            // Custom css from meta data
            var s = d.createElement("style");
            s.innerText = metaData.customCss;
            d.head.appendChild(s);
            // UI Font Family Logic
            ...
            ClickAudit_util_1.ClickAuditUtil.setUpLinkAuditEvents(win, metaData === null || metaData === void 0 ? void 0 : metaData.transportPod);
            ClickAudit_util_1.ClickAuditUtil.setUpVideoAuditEvents(win, metaData === null || metaData === void 0 ? void 0 : metaData.transportPod);
            //ensure the iframe element on the parent page has the same height as its content inside
            var checkHeight = function () {
                try {
                    var br = pageWrap.getBoundingClientRect();
                    contentiFrame.style.height = br.height + 'px';
                    typeof onResizeCallback === 'function' && onResizeCallback();
                }
                catch (ex) {
                    console.error(ex);
    ...
    FrameBuilder.prototype.appendWalkthroughStepNumber = function (container, styleConfig, sequence, totalSteps) {
        ...
        if (!shouldShowStepNumber || !stepNumberColor)
            return;
        var numberWrap = this.windowSvc.document.createElement('span'), num = (sequence || 0) + 1;
    ...
    FrameBuilder.prototype.addWalkthroughStyleProperties = function (frame, config, stepType, styleConfig, targetElement) {
        ...
        this.addAnnouncementStyleProperties(frame, targetElement, styleId, styleProps, Enums_1.Enums.FrameType.WalkThrough, announcementStyleConfig);
    };
    /* Shared Utilities */
    ...
    FrameBuilder.prototype.fixVideoElementsFromFroala = function (videos, parentWidth) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            var vidWidth = vidProps.width || parentWidth - 42, h = ratio * vidWidth;
...
var Herald = /** @class */ (function () {
    function Herald(appSvc, tenantSvc, sessionSvc, messageSvc, walkthroughSvc, windowSvc, commSvc, templateCache) {
        ...
        this.newSurveyAnnouncements = false;
    ...
    Herald.prototype.onInit = function () {
        var _this = this;
        this.newSurveyAnnouncements = this.tenantSvc.FeatureEnabled(tenant_service_1.ReleaseGates.NewSurveyAnnouncements);
    ...
    Herald.prototype.ShowNPSClickHandler = function (e) {
        try {
            ...
            this.closeSlider();
            this.showNPSPopup(url, showMessageBubble, wide);
        }
        catch (ex) {
            //report?
    ...
    Herald.prototype.messageClicked = function (id) {
        ...
        console.log("Announcement Clicked: ", theId);
    ...
    Herald.prototype.addAdvancedDisplayAnnouncement = function (announcement) {
        ...
        switch (announcementType) {
            case Enums_1.Enums.AnnouncementType.Basic:
                if (announcement.announcementPositionId === Enums_1.Enums.AnnouncementPosition.PopUp) {
                    if (this.tenantSvc.FeatureEnabled(tenant_service_1.ReleaseGates.InAppContentV2)) {
                ...
                break;
            case Enums_1.Enums.AnnouncementType.Nps:
                this.messageSvc.normalizeMessageData(announcement);
                if (announcement.announcementPositionId === Enums_1.Enums.AnnouncementPosition.PopUp) {
                    this.showNPSPopup(announcement.messageData.responseUrl, announcement.showInfoMessageString, announcement.messageData.wide);
                }
                else if (announcement.announcementPositionId > Enums_1.Enums.AnnouncementPosition.PopUp) {
                    if (this.newSurveyAnnouncements) {
                        this.showNPSSlider(announcement);
                    }
                    else {
                        this.showNPSSlider_old(announcement);
                    }
                }
                break;
            default:
    ...
    Herald.prototype.showNPSSlider_old = function (announcement) {
        var announcementPositionClass = Herald.getAnnouncementPositionClassFromId(announcement.announcementPositionId);
        ...
        announcement.AdditionalScoresClass = announcement.messageData.wide ? 'scores-wide' : '';
        ...
        announcement.Buttons = this.generateSurveyButtons(announcement.messageData) || this.generateDefaultNpsButtons(announcement.messageData);
    ...
    Herald.prototype.showNPSSlider = function (socketAnnouncement) {
        var _this = this;
        var _a;
        ...
        messageStyleConfig = ((_a = announcement.messageData) === null || _a === void 0 ? void 0 : _a.styleConfig) || {}, configuration = {
            ...
            onLoadCallback: function (ctx) {
                ...
                link.type = 'text/css';
                link.href = styleUrl;
                ctx.document.getElementsByTagName('head')[0].prepend(link);
            },
            onClickCallback: function (e) {
                ...
                if (hasClass) {
                    try {
                        var url = target.getAttribute("data-nps-url");
                        var wide = (target.getAttribute('data-nps-wide') || '').toLowerCase() === 'true';
                        var announcement_1 = clientlibs_1.CoreUtils.closest(target, ".cz-announcement");
                        var showMessageBubble = (announcement_1.getAttribute("data-info-message") || "true").toLowerCase() === "true";
                        _this.closeSlider();
                        _this.showNPSPopup(url, showMessageBubble, wide);
                    }
                    catch (ex) {
                        //report?
                        errorLogger_1.default.report("Herald.npsClickEvent", ex);
    ...
    Herald.prototype.showWalkthroughSlider = function (content, config, clickAuditData, styleConfig, sequence, metaData) {
        var _this = this;
        var announcementPositionClass = Herald.getAnnouncementPositionClassFromId(config.position), container = this.windowSvc.document.createElement("div");
    ...
    Herald.prototype.showInAppPopup = function (announcement) {
        ...
        //const container: HTMLDivElement = this.windowSvc.document.createElement("div");
        //container.classList.add(this.inAppPopupClass);
        //let popupContentContainer = this.windowSvc.document.createElement("div");
        //popupContentContainer.classList.add(this.inAppPopupContentClass);
    ...
    Herald.prototype.showNPSPopup = function (url, infoMessageString, wide) {
        var _this = this;
        if (!this.newSurveyAnnouncements) {
            this.showNPSPopup_old(url, infoMessageString, wide);
            return;
        }
        var surveyFrame = this.frameBuilder.buildSurveyContentFrame(url, infoMessageString);
        var globalStyleConfig = this.sessionSvc.CurrentAccountContent.announcementStyles;
        var width = 717, //globalStyleConfig?.sliderWidth,
        messageStyleConfig = {}, configuration = {
            ...
            onCloseCallback: function () {
                _this.closePopUpAnnouncement(false, false);
                var showMessageBubble = (infoMessageString || "true").toLowerCase() === "true";
                if (showMessageBubble)
                    _this.announceClosure(_this.sessionSvc.CurrentAccountContent.clickToSeeMoreText);
            },
            onLoadCallback: function (ctx) {
    ...
    Herald.prototype.showNPSPopup_old = function (url, infoMessageString, wide) {
        var surveyFrame = this.frameBuilder.buildSurveyContentFrame(url, infoMessageString);
        this.showPopUpAnnouncement(surveyFrame, wide ? "800px" : "640px");
        //Add NPS class
        this.popupDiv.classList.add("nps");
    };
    ...
    Herald.prototype.showWalkthroughPopup = function (content, config, clickAuditData, styleConfig, sequence, metaData) {
        var _this = this;
        //build the popup content container
        var container = this.windowSvc.document.createElement("div");
        ...
        if (!this.windowSvc.document.getElementById(FrameBuilder_1.FRAME_BUILDER_STYLE_ELEMENT_ID_MAPPING.styleOverrides)) {
            var styleOverrides = this.windowSvc.document.createElement('style');
    ...
    Herald.prototype.showPopUpAnnouncement = function (content, popupWidth) {
        // SHOW THE BG
        var transBg = this.windowSvc.document.getElementById('cz_transBG');
        ...
        if (!this.newSurveyAnnouncements) {
            var frame_1 = this.windowSvc.czq("#cz_nps_frame"), setFrameHeight_1 = function () {
                /*let popOver = document.getElementById("cz_popover"),
                    ph = popOver.clientHeight - 40;*/
                var ch = parseInt(frame_1.attr("data-content-size")), mx = ((window.innerHeight
                    || document.documentElement.clientHeight
                    || document.body.clientHeight) * 0.75) - 45;
                var heightValueToUse = ch + 1;
                //if the content height is greater than the max height
                ...
                frame_1.height(heightValueToUse + "px");
            };
            if (frame_1.length > 0) {
                ...
                this.windowSvc.elementCache.window.on(this.windowSvc.namespaceEvent("message.npsPopup"), function (e) {
                    try {
                        if (clientlibs_1.CoreUtils.hasOwnProp(e.originalEvent, 'data') || !e.originalEvent.data.height) {
                            return;
                        }
                        var fh = parseInt(e.originalEvent.data.height); // - 24;
                        frame_1.attr("data-content-size", fh);
                        setFrameHeight_1();
                    }
                    catch (ex) {
                        //report?
    ...
    Herald.prototype.closePopUpAnnouncement = function (wasFromWalkthrough, wasInCTA) {
        var popupBg = this.popupBackground;
        if (popupBg) {
            // HIDE THE BG
        ...
        if (popupDiv) {
            // HIDE THE DIV and remove any style classes
            popupDiv.classList.remove('shown', 'walk', 'nps', this.inAppPopupClass);
            //reset any styles
            this.popupDiv.style.borderRadius = "";
            this.popupDiv.style.backgroundColor = "";
        }
    ...
    Herald.prototype.showTooltip = function (content, iconfig, clickAuditData, retryNumber, styleConfig, sequence, metaData) {
        ...
        target.each(function (i, t) {
            ...
            var container = _this.windowSvc.document.createElement("div");
            ttConfig.wasScrolled = false;
            container.classList.add(_this.tooltipClass);
            container.classList.add("cz-click-audit");
            // ! Old logc here. Probably not needed anymore
            // if (containerHasBorder) {
            //     container.classList.add(`${this.tooltipClass}-with-border`);
            // }
            var tooltipContentContainer = _this.windowSvc.document.createElement("div"), calcFunc = function () {
                _this.calcTooltipPosition(ttConfig, t, container, i === 0, styleConfig);
            };
            // ! Old logic here. Probably not needed anymore
            // if (hasRadius && !containerHasBorder) {
            //     tooltipContentContainer.style.borderRadius = `${borderRadius}px`;
            //     tooltipContentContainer.style.overflow = 'hidden';
            // }
            ...
            // ! Old logic here. Probably not needed anymore
            // tooltipContentContainer.style.borderColor = backgroundColor || '#fff';
            // tooltipContentContainer.style.backgroundColor = backgroundColor || "#fff";
            ...
            var body = (ttConfig.container === Enums_1.Enums.ContainerType.Parent) ?
                t.parentElement :
    ...
    Herald.prototype.calcTooltipPosition = function (config, t, frame, scrollToView, styleConfig) {
        var _a = styleConfig || {}, borderWidth = _a.borderWidth, borderColor = _a.borderColor, backgroundColor = _a.backgroundColor, ctaBackgroundColor = _a.ctaBackgroundColor, shouldShowStepNumber = _a.shouldShowStepNumber, stepNumberPosition = _a.stepNumberPosition, hasBorder = ((borderWidth || 0) > 0 && typeof borderColor === 'string'), bgColor = (hasBorder) ? borderColor : backgroundColor || config.backgroundColor, ctaBgColor = ctaBackgroundColor || config.ctaBackgroundColor;
        // ! Not sure the below variable is needed anymore, but keep it around for reference.
        //hasTopStepNumber = shouldShowStepNumber && (stepNumberPosition === StepNumberPosition.TopLeft || stepNumberPosition === StepNumberPosition.TopCenter || stepNumberPosition === StepNumberPosition.TopRight);
        ...
        // ! Do not believe this logic is needed anymore
        // if (config.borderRadius) {
        //     let topPadding = (config.borderRadius / 4);
        //     if (topPadding < 20) {
        //         topPadding = 20;
        //     }
        //     if (topPadding > 150) {
        //         topPadding = 150;
        //     }
        //     // Make extra room for the step number
        //     if (hasTopStepNumber)
        //         topPadding += 5;
    ...
    Herald.prototype.generateDefaultNpsButtons = function (messageData) {
        var _this = this;
        var valueParam = messageData.responseUrl.indexOf('?') === -1 ? '?value=' : '&value=';
        return __spreadArrays((Array(11))).map(function (v, i) { return _this.appSvc.templater.populateTemplate(_this.templateCache.announcementSurveyButtonTemplate, {
            Style: '',
            TextStyle: '',
...
var BasePanelDisplay = /** @class */ (function () {
    ...
    BasePanelDisplay.prototype.showPanelToggle = function () {
        this.panelToggle.removeClass('hidden');
        //this.panelToggle.attr("style", StyleUtils.replaceStyleValue("none", "block"));
    };
    BasePanelDisplay.prototype.hidePanelToggle = function () {
        this.panelToggle.addClass('hidden');
        //this.panelToggle.attr("style", StyleUtils.replaceStyleValue("block", "none"));
    };
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.PubSubSubscription = exports.PubSubMessage = void 0;
var errorLogger_1 = __importDefault(__webpack_require__(/*! ./errorLogger */ "./src/core/errorLogger.ts"));
var clientlibs_1 = __webpack_require__(/*! @churnzero/clientlibs */ "./node_modules/@churnzero/clientlibs/dist/main/index.js");
...
var PubSuber = /** @class */ (function () {
    ...
    PubSuber.removeAll = function (subIds) {
        ...
        for (var key in subIds) {
            if (!clientlibs_1.CoreUtils.hasOwnProp(subIds, key)) {
                continue;
            }
            if (subIds[key]) {
                subIds[key].remove();
...
var SuccessPanel = /** @class */ (function (_super) {
    ...
    SuccessPanel.prototype.applyBranding = function (branding) {
        ...
        var mapBrandingItemToCSSProps = function (brandingItem) {
            ...
            for (var k in brandingItem) {
                if (clientlibs_1.CoreUtils.hasOwnProp(brandingItem, k) && brandingItem[k] != null && (brandingItem[k] || '').trim().length > 0) {
                    rules.push(clientlibs_1.StyleUtils.camelCaseToDash(k) + ':' + brandingItem[k] + ' !important;');
                }
            }
            return rules;
        };
        var inheritValue = function inheritValueFn(to, property, from) {
            if (!to || !from || !property) {
                return;
            }
        ...
        if (branding.panel) {
            ...
            if (branding.panel.backgroundColor) {
                branding.header = branding.header || {};
                inheritValue(branding.header, 'backgroundColor', branding.panel);
                branding.accountRepPane = branding.accountRepPane || {};
                inheritValue(branding.accountRepPane, 'backgroundColor', branding.panel);
                branding.announcements = branding.announcements || {};
                inheritValue(branding.announcements, 'backgroundColor', branding.panel);
                branding.footer = branding.footer || {};
                inheritValue(branding.footer, 'backgroundColor', branding.panel);
            }
        }
    ...
    SuccessPanel.prototype.onDigest = function () {
        ...
        // hide the toggle
        if (zeroAnnouncements && this.sessionSvc.OnlyShowWhenMessagesExist && !this.sessionSvc.ContentPanelEnabled) {
            this.panelToggle.addClass('hidden');
        }
        else {
            this.panelToggle.removeClass('hidden');
...
var DevlessPanelLauncher_1 = __importDefault(__webpack_require__(/*! @src/core/DevlessPanelLauncher */ "./src/core/DevlessPanelLauncher.ts"));
...
var ChurnZeroApp = /** @class */ (function () {
    function ChurnZeroApp() {
        ...
        this.version = "localhost-2020-11-11T19:44:30.011Z"; //gets replaced by build task
    ...
    ChurnZeroApp.prototype.halt = function () {
        ...
        // unload content (admin) panel so iframe is removed
        this.unloadContentPanel();
        // Unload the success panel (this will also unload herald)
    ...
    ChurnZeroApp.prototype.bootMasterFrame = function () {
        ...
        if (this.sfSubIds) {
            for (var key in this.sfSubIds) {
                if (clientlibs_1.CoreUtils.hasOwnProp(this.sfSubIds, (key))) {
    ...
    ChurnZeroApp.prototype.registerChurnZeroEventCallback = function (type, callback) {
        ...
        if (!this.eventHandlers || !clientlibs_1.CoreUtils.hasOwnProp(this.eventHandlers, type)) {
            return;
        }
    ...
    ChurnZeroApp.prototype.deregisterChurnZeroEventCallback = function (type, callback) {
        ...
        type = type.toLowerCase();
        if (!this.eventHandlers || !clientlibs_1.CoreUtils.hasOwnProp(this.eventHandlers, type) || this.eventHandlers[type].length) {
            return;
        }
        //remove all
        if (clientlibs_1.CoreUtils.isNullOrUndefined(callback)) {
    ...
    ChurnZeroApp.prototype.startKernel = function () {
        ...
        try {
            ...
            var currentContact = this.tenantSvc.contact;
            if (currentContact && clientlibs_1.CoreUtils.hasOwnProp(currentContact, "id") && currentContact.id !== null) {
        ...
        if (this.isMasterFrame) {
            if (!this.dbDisconnected && !this.dbMigrating) {
                try {
                    ...
                    else if (!this.sessionSvc.PanelEnabled && this.sessionSvc.ContentPanelEnabled) {
                        this.loadDevlessLauncher();
                    }
                }
                catch (ex) {
    ...
    ChurnZeroApp.prototype.loadDevlessLauncher = function () {
        //setup the Success Panel
        this._successPanel = new DevlessPanelLauncher_1.default(this.applicationSvc, this.tenantSvc, this.windowSvc, this.storageSvc, this.sessionSvc, this.messageSvc, this.commSvc, this.wasOpenRequested);
    };
    ChurnZeroApp.prototype.unloadDevlessLauncher = function () {
        if (this._successPanel) {
            if (clientlibs_1.CoreUtils.hasOwnProp(this._successPanel, 'onHalt') && typeof this._successPanel['onHalt'] === 'function') {
                this._successPanel['onHalt']();
            }
    ...
    ChurnZeroApp.prototype.loadPageMonitor = function () {
        ...
        this.applicationSvc.pubsub.subscribe(Enums_1.Enums.ChurnZeroAppEvents.DIGEST, function () {
    ...
    ChurnZeroApp.prototype.loadHerald = function () {
        ...
        var panelEnabled = this.sessionSvc.PanelEnabled;
        ...
        /** SuccessPanel is enabled. No need to load announcement styles. **/
        if (panelEnabled) {
            return;
        }
        /** SuccessPanel is not enabled -- Load announcement styles explicitly. **/
        // TODO: When announcments are fully decoupled from SuccessPanel, we should load those styles separately from the csm_panel_styles
        var styleUrl = this.applicationSvc.addCdn('css/announcements.css'), link = this.windowSvc.document.createElement('link');
        link.id = CZ_ANNOUNCEMENT_STYLES_ID;
        link.rel = 'stylesheet';
        link.type = 'text/css';
        link.href = styleUrl;
...
var ErrorLogger = /** @class */ (function () {
    function ErrorLogger(reportUrl, coder, dateConstructor) {
        ...
        //AppVersion
        this._appVersion = "localhost-2020-11-11T19:44:30.011Z"; //gets replaced by build task
        if (!this.reportUrl) {
        ...
        for (var i = 0; i < queLength; i++) {
            ...
            switch (socketMessage.socketMessageTypeId) {
                ...
                case Enums_1.Enums.SocketMessageTypes.SOCKETMESSAGETYPE_CHECK_EVENT_MAP_CONFIG_VERSION:
                    ...
                    break;
                case Enums_1.Enums.SocketMessageTypes.SOCKETMESSAGETYPE_CheckPageTagConfigVersion:
...
var MessageService = /** @class */ (function () {
    ...
    MessageService.prototype.getAnnouncementType = function (announcement) {
        if (!announcement.messageData) {
            return Enums_1.Enums.AnnouncementType.Basic;
        }
        ...
        var matchingKeys = Object.keys(Enums_1.Enums.AnnouncementType).filter(function (key) { return key.toString().toLowerCase() === (enumString || '').toLowerCase(); });
        if (matchingKeys.length > 0) {
            var enumKey = matchingKeys[0];
            return Enums_1.Enums.AnnouncementType[enumKey];
        }
        return undefined;
    };
    MessageService.prototype.isNPSAnnouncement = function (announcement) {
        ...
        return this.getAnnouncementType(announcement) === Enums_1.Enums.AnnouncementType.Nps;
    };
...
var SessionService = /** @class */ (function () {
    ...
    Object.defineProperty(SessionService.prototype, "LaunchAvatarUrl", {
        ...
        get: function () {
            ...
            if (!this.PanelEnabled && this.ContentPanelEnabled) {
...
(function (ReleaseGates) {
    ReleaseGates["InAppContentV2"] = "inAppContentV2";
    ReleaseGates["NewSurveyAnnouncements"] = "newSurveyAnnouncementsEnabled";
    ReleaseGates["SuccessPanelsV2"] = "successPanelsV2";
...
var TenantService = /** @class */ (function () {
    function TenantService(storageSvc) {
        ...
        this.featureFlags[ReleaseGates.InAppContentV2] = false;
        this.featureFlags[ReleaseGates.SuccessPanelsV2] = false;
    }
    //************************//
    // Tenant Id
    //************************//
    ...
    TenantService.prototype.buildUIFontFamilyCssRequestUrlFromConfig = function (styleConfig) {
        ...
        var fontFamily = styleConfig.fontFamily, uiFontFamilyIds = styleConfig.uiFontFamilyIds, arr = (uiFontFamilyIds || []).concat([]), queryParts = [];
        var ffId = this.getUIFontFamilyMappingId(fontFamily);
        if (ffId) {
            arr.push(ffId);
        }
        var ctaFfId = null;
        if (styleConfig && clientlibs_1.CoreUtils.hasOwnProp(styleConfig, 'ctaFontFamily')) {
            ctaFfId = this.getUIFontFamilyMappingId(styleConfig.ctaFontFamily);
        }
        if (ctaFfId) {
            arr.push(ctaFfId);
        }
            ...
            for (var i = 0; i < walkthroughConfig.items.length; i++) {
                if (walkthroughConfig.items[i]) {
                    ...
                    if (properties === null || properties === void 0 ? void 0 : properties.styleConfigsEnabled) {
                        ...
                        var defaults = __assign(__assign({}, clientlibs_1.CZ_DEFAULT_STYLE_CONFIG), tenantStyleConfig);
                        this.hydateStepStyleConfigs(defaults, walkthroughConfig.items[i]);
                    }
                }
            }
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.ClickAuditUtil = void 0;
var clientlibs_1 = __webpack_require__(/*! @churnzero/clientlibs */ "./node_modules/@churnzero/clientlibs/dist/main/index.js");
var ClickAuditUtil = /** @class */ (function () {
    function ClickAuditUtil() {
    }
    /**
     ...
     */
    ClickAuditUtil.setUpLinkAuditEvents = function (frameWindow, transportPod) {
        ...
        frameDocument.addEventListener('click', function (e) {
            ...
            if (sendAudit) {
                ...
                ClickAuditUtil.postEncryptedMessage(frameWindow.parent, msg);
            }
        });
    };
    /**
     ...
     * @param transportPod the transport message to send upon event dispatch
     */
    ClickAuditUtil.setUpVideoAuditEvents = function (frameWindow, transportPod) {
        var frameDocument = frameWindow.document;
        ...
        frameWindow.addEventListener('blur', function (e) {
            ...
            ClickAuditUtil.postEncryptedMessage(frameWindow.parent, __assign({}, transportPod));
        });
        frameWindow.addEventListener("message", function (e) {
            ...
            ClickAuditUtil.postEncryptedMessage(sourceWindow, __assign({}, transportPod), origin);
        });
    };
    /**
     ...
     */
    ClickAuditUtil.postEncryptedMessage = function (targetWindow, message, origin) {
        var em = clientlibs_1.iFrameUtils.encrypt(JSON.stringify(message));
        targetWindow.postMessage(em, origin || '*');
    };
    return ClickAuditUtil;
}());
exports.ClickAuditUtil = ClickAuditUtil;
